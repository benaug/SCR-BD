...
source("simSCRBD.R")
source("init.data.R")
source("NimbleModel SCRBD Bernoulli BdayConstraints.R")
...
#We might have additional information on age
#Here, we can simulate deterministic constraints on
#the birthday as you might have from a human observer.
#e.g., you may be able to tell an individual is at least
#x days old and at most y days old.

bday.delta=4 #window of deterministic constraints in occasions units. Simulated at random to contain the true birthday

data=simSCRBD(B=B,p0=p0,sigma=sigma,X=X,buff=3,obstype="bernoulli",
             omega=omega,t.mu=t.mu,t.sd=t.sd,K=K,K2D=K2D,bday.delta=bday.delta)
...
Nimdata<-list(y=nimbuild$y,b=b.data,first.seen=nimbuild$first.seen,last.seen=nimbuild$last.seen,
              bday.constraint.data=rep(1,M),bday.range=nimbuild$bday.range) #bday.range augmented
...
#Can see bday's obeying constraints.
mvSamples2 = as.matrix(Cmcmc$mvSamples2)
idx=grep("bday",colnames(mvSamples2))
